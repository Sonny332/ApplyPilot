...
_ROLE_FAMILY_PATTERNS: dict[str, tuple[str, ...]] = {
        "energy_analytics_utility": (
        r"\benergy analyst\b",
        r"\benergy analytics?\b",
        r"\benergy analysis\b",
        r"\benergy market analyst\b",
        r"\benergy pricing analyst\b",
        r"\bpricing analyst\b",
        r"\bmarket analyst\b",
        r"\butility analyst\b",
        r"\butility program analyst\b",
        r"\bdsm analyst\b",
        r"\bdemand[\s-]?side management\b",
        r"\benergy consultant\b",
        r"\bquantitative energy consultant\b",
        r"\btariff\b",
        r"\brate design\b",
        r"\btime[\s-]?of[\s-]?use\b",
        r"\bTOU\b",
        r"\bISO[\s-]?NE\b",
        r"\benergy economics\b",
    ),
    "building_energy_efficiency": (
        r"\benergy efficiency engineer\b",
        r"\benergy engineer\b",
        r"\bbuilding energy\b",
        r"\bbuilding energy analyst\b",
        r"\bbuilding performance\b",
        r"\bbuilding performance analyst\b",
        r"\benergy audit\b",
        r"\benergy auditor\b",
        r"\binvestment[\s-]?grade audit\b",
        r"\bretrofit\b",
        r"\bretrofit analysis\b",
        r"\bdecarbonization\b",
        r"\bdecarbonisation\b",
        r"\bbuilding decarbonization\b",
        r"\bbuilding decarbonisation\b",
        r"\benergy modeling\b",
        r"\benergy modelling\b",
        r"\bM&V\b",
        r"\bmeasurement and verification\b",
    ),
    "hvac_building_systems": (
        r"\bHVAC\b",
        r"\bheat pump\b",
        r"\bheat[\s-]?pump\b",
        r"\bbuilding systems\b",
        r"\bmechanical systems\b",
        r"\bHVAC optimization\b",
        r"\bHVAC optimisation\b",
        r"\bHVAC performance\b",
        r"\bbuilding controls\b",
        r"\bbuilding automation\b",
        r"\bBAS\b",
        r"\bBMS\b",
        r"\benergy management system\b",
        r"\bretro[\s-]?commissioning\b",
        r"\bcommissioning\b",
    ),
    "renewable_storage_analysis": (
        r"\brenewable energy analyst\b",
        r"\brenewable energy consultant\b",
        r"\bsolar analyst\b",
        r"\bsolar energy analyst\b",
        r"\bsolar PV\b",
        r"\bphotovoltaic\b",
        r"\bPV\b",
        r"\bbattery storage\b",
        r"\benergy storage\b",
        r"\bstorage analyst\b",
        r"\bdistributed energy resources\b",
        r"\bDER\b",
        r"\bSMART incentive\b",
        r"\bnet metering\b",
    ),
    "data_center_energy_thermal": (
        r"\bdata center energy\b",
        r"\bdata centre energy\b",
        r"\bdata center thermal\b",
        r"\bdata centre thermal\b",
        r"\bdata center cooling\b",
        r"\bdata centre cooling\b",
        r"\bdata center facilities engineer\b",
        r"\bdata centre facilities engineer\b",
        r"\bdata center facility engineer\b",
        r"\bdata centre facility engineer\b",
        r"\bdata center mechanical engineer\b",
        r"\bdata centre mechanical engineer\b",
        r"\bthermal engineer\b",
        r"\bcooling engineer\b",
        r"\bliquid cooling\b",
        r"\bdirect[\s-]?to[\s-]?chip cooling\b",
        r"\bimmersion cooling\b",
        r"\bmission critical facilities\b",
        r"\bPUE\b",
        r"\bpower usage effectiveness\b",
    ),
...
_SKILL_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ...
    ("nlp", re.compile(r"\bnlp\b|\bnatural language processing\b", re.IGNORECASE)),
        # Energy analytics / utility / market analysis
    ("energy efficiency", re.compile(r"\benergy efficiency\b", re.IGNORECASE)),
    ("energy analytics", re.compile(r"\benergy analytics?\b|\benergy analysis\b", re.IGNORECASE)),
    ("utility analysis", re.compile(r"\butility analysis\b|\butility analyst\b|\butility program\b", re.IGNORECASE)),
    ("demand-side management", re.compile(r"\bdemand[\s-]?side management\b|\bDSM\b", re.IGNORECASE)),
    ("tariff analysis", re.compile(r"\btariff(?:s)?\b|\brate design\b|\btime[\s-]?of[\s-]?use\b|\bTOU\b", re.IGNORECASE)),
    ("ISO-NE", re.compile(r"\bISO[\s-]?NE\b|\bISO New England\b", re.IGNORECASE)),
    ("energy economics", re.compile(r"\benergy economics\b|\benergy market\b|\benergy pricing\b", re.IGNORECASE)),

    # Building energy / decarbonization / efficiency
    ("energy audit", re.compile(r"\benergy audit(?:s)?\b|\benergy auditor\b|\binvestment[\s-]?grade audit\b", re.IGNORECASE)),
    ("building performance", re.compile(r"\bbuilding performance\b|\bbuilding energy\b", re.IGNORECASE)),
    ("retrofit analysis", re.compile(r"\bretrofit(?:s)?\b|\bretrofit analysis\b|\bretrofit economics\b", re.IGNORECASE)),
    ("decarbonization", re.compile(r"\bdecarboni[sz]ation\b|\bbuilding decarboni[sz]ation\b", re.IGNORECASE)),
    ("energy modeling", re.compile(r"\benergy model(?:ing|ling)?\b|\bbuilding energy model(?:ing|ling)?\b", re.IGNORECASE)),
    ("measurement and verification", re.compile(r"\bM&V\b|\bmeasurement and verification\b", re.IGNORECASE)),
    ("utility incentives", re.compile(r"\butility incentive(?:s)?\b|\brebate(?:s)?\b|\bincentive program(?:s)?\b", re.IGNORECASE)),

    # HVAC / building systems
    ("HVAC", re.compile(r"\bHVAC\b|\bheating, ventilation, and air conditioning\b", re.IGNORECASE)),
    ("heat pumps", re.compile(r"\bheat[\s-]?pump(?:s)?\b", re.IGNORECASE)),
    ("building systems", re.compile(r"\bbuilding system(?:s)?\b|\bmechanical system(?:s)?\b", re.IGNORECASE)),
    ("building controls", re.compile(r"\bbuilding control(?:s)?\b|\bbuilding controls?\b|\bcontrol systems?\b", re.IGNORECASE)),
    ("building automation", re.compile(r"\bbuilding automation\b|\bBAS\b|\bBMS\b", re.IGNORECASE)),
    ("commissioning", re.compile(r"\bcommissioning\b|\bretro[\s-]?commissioning\b|\bRCx\b", re.IGNORECASE)),
    ("energy management systems", re.compile(r"\benergy management system(?:s)?\b|\bEMS\b", re.IGNORECASE)),

    # Renewable / solar / storage
    ("renewable energy", re.compile(r"\brenewable energy\b|\bclean energy\b", re.IGNORECASE)),
    ("solar PV", re.compile(r"\bsolar PV\b|\bphotovoltaic(?:s)?\b|\bPV system(?:s)?\b", re.IGNORECASE)),
    ("battery storage", re.compile(r"\bbattery storage\b|\benergy storage\b|\bBESS\b", re.IGNORECASE)),
    ("distributed energy resources", re.compile(r"\bdistributed energy resources\b|\bDER\b|\bDERs\b", re.IGNORECASE)),
    ("SMART incentives", re.compile(r"\bSMART incentive(?:s)?\b|\bSMART program\b", re.IGNORECASE)),
    ("net metering", re.compile(r"\bnet metering\b", re.IGNORECASE)),

    # Data center energy / cooling / thermal infrastructure
    ("data center cooling", re.compile(r"\bdata cent(?:er|re) cooling\b|\bcooling infrastructure\b", re.IGNORECASE)),
    ("data center facilities", re.compile(r"\bdata cent(?:er|re) facilit(?:y|ies)\b|\bmission critical facilit(?:y|ies)\b", re.IGNORECASE)),
    ("data center thermal management", re.compile(r"\bdata cent(?:er|re) thermal\b|\bthermal management\b", re.IGNORECASE)),
    ("liquid cooling", re.compile(r"\bliquid cooling\b|\bdirect[\s-]?to[\s-]?chip cooling\b|\bDTC cooling\b", re.IGNORECASE)),
    ("immersion cooling", re.compile(r"\bimmersion cooling\b", re.IGNORECASE)),
    ("rack power density", re.compile(r"\brack power density\b|\bhigh[\s-]?density racks?\b|\bkW per rack\b", re.IGNORECASE)),
    ("PUE", re.compile(r"\bPUE\b|\bpower usage effectiveness\b", re.IGNORECASE)),
    ("CRAC CRAH", re.compile(r"\bCRAC\b|\bCRAH\b|\bcomputer room air conditioning\b|\bcomputer room air handler\b", re.IGNORECASE)),
    ("chilled water systems", re.compile(r"\bchilled water\b|\bchiller(?:s)?\b|\bcooling tower(?:s)?\b", re.IGNORECASE)),
    ("UPS power systems", re.compile(r"\bUPS\b|\buninterruptible power supply\b", re.IGNORECASE)),
    ("critical power", re.compile(r"\bcritical power\b|\bbackup power\b|\bgenerator(?:s)?\b", re.IGNORECASE)),

    # Facilities / technical operations as supporting evidence, not broad role-family boost
    ("facilities operations", re.compile(r"\bfacilit(?:y|ies) operations\b|\bbuilding operations\b", re.IGNORECASE)),
    ("vendor coordination", re.compile(r"\bvendor coordination\b|\bcontractor coordination\b", re.IGNORECASE)),
    ("inspection readiness", re.compile(r"\binspection readiness\b|\bcode compliance\b|\bcompliance[\s-]?sensitive\b", re.IGNORECASE)),
    ("scope definition", re.compile(r"\bscope definition\b|\bscope of work\b|\bSOW\b", re.IGNORECASE)),

    # Engineering foundation / tools from candidate evidence
    ("financial modeling", re.compile(r"\bfinancial model(?:ing|ling)?\b|\bIRR\b|\bNPV\b|\bROI\b", re.IGNORECASE)),
    ("scenario analysis", re.compile(r"\bscenario analysis\b|\bsensitivity analysis\b", re.IGNORECASE)),
    ("Power Query", re.compile(r"\bPower Query\b", re.IGNORECASE)),
    ("Excel", re.compile(r"\bExcel\b|\bspreadsheet(?:s)?\b", re.IGNORECASE)),
...
def _compute_deterministic_baseline(scoring_profile: dict, job: dict) -> dict:
    ...
    role_family_bonus = (
        1.0
        if profile_family == job_family and profile_family in _BONUS_ROLE_FAMILIES
        else 0.0
    )
